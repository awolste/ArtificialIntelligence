...
###############################################################################
    ...
    def max_value(player, board, depth_limit):
        ...
        for s in successors:
            ...
            temp_v = value(next_player, s[1], depth_limit)
            if v < temp_v[0]:
                v = temp_v[0]
                best_move = s[0]
        return v, best_move
    ...
    def min_value(player, board, depth_limit):
        v = math.inf
        next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
        successors = get_child_boards(next_player, board)
        for s in successors:
            temp_v = value(player, s[1], depth_limit - 1)
            if v > temp_v[0]:
                v = temp_v[0]
                best_move = s[0]
        return v, best_move
...
###############################################################################
    ...
    def max_value(player, board, depth_limit, alpha, beta):
        ...
        for s in successors:
            next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
            temp_v = value(next_player, s[1], depth_limit, alpha, beta)
            if v < temp_v[0]:
                v = temp_v[0]
                best_move = s[0]
                alpha = max(alpha, v)
    ...
    def min_value(player, board, depth_limit, alpha, beta):
        ...
        successors = get_child_boards(next_player, board)
        for s in successors:
            temp_v = value(player, s[1], depth_limit - 1, alpha, beta)
            if v > temp_v[0]:
                v = temp_v[0]
                best_move = s[0]
                beta = min(beta, v)
    ...
    placement = value(player, board, depth_limit, a, b)[1]
###############################################################################
    return placement
...
###############################################################################
    ...
    def exp_value(player, board, depth_limit):
        ...
        for s in successors:
            count += 1
            next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
            v += value(next_player, s[1], depth_limit - 1)[0]
        v /= count  # this allows us to see the average value across all moves possible
        return v, None
