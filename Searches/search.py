    ...
    while not len(open_set) == 0:
        ...
        if n_rows > temp[0] >= 0:
            ...
            if child not in open_set and child not in closed_set and child not in obstacles:
                ...
                if child == goal:
                    movement = construct_movement(start_row, start_col, goal_row, goal_col, actions, parent)
...
def uniform_cost_search(grid_size, start, goal, obstacles, costFn, logger):
    ...
    # ----------------------------------------
#############################################################################
    open_set.put(start, costFn(start))
    while not len(open_set) == 0:
        node = open_set.pop()
        if node[0] == goal:
            movement = construct_movement(start_row, start_col, goal_row, goal_col, actions, parent)
            return movement, closed_set
        closed_set.put(node[0], costFn(node[0]))
        ...
        if n_rows > tup[0] >= 0:
            ...
            if child[0] not in open_set and child[0] not in closed_set and child[0] not in obstacles:
                open_set.put(child[0], costFn(child[0]))
...
#############################################################################
    ...
    while not len(open_set) == 0:
        ...
        if n_rows > tup[0] >= 0:
            ...
            child = [temp, node[1]]
            if child[0] not in open_set and child[0] not in closed_set and child[0] not in obstacles:
                open_set.put(child[0], costFn(child[0]) + heuristic(tup[0], tup[1]))
                actions[tup[0]][tup[1]] = (1, 0)
                parent[tup[0]][tup[1]] = node[0]
            elif child[0] in open_set:
                if open_set.get(child[0]) > costFn(child[0]):
...
def construct_movement(starting_r, starting_c, ending_r, ending_c, actions, parent):
    """
    Parameters
    ----------
    starting_r  row index of starting node

    starting_c  col index of starting node

    ending_r    row index of ending node

    ending_c     col index of ending node

    actions      matrix of actions, none if position was never travered

    parent       matrix of parents of current node
    ...
    """
    temp_array = []
    x, y = ending_r, ending_c
    while x != starting_r or y != starting_c:
        if actions[x][y] is None:
            return temp_array
        temp_array.append(actions[x][y])
        tup = actions[x][y]
        x -= tup[0]
        y -= tup[1]
    temp_array.reverse()
