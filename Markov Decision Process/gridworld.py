    ...
    for k in range(1, max_iterations):
        for s in range(NUM_STATES):
            ...
            for a in range(NUM_ACTIONS):
                ...
                # Vk(s) = max(P(s_ | s, a) * R(s, a, s_) + gamma* Vk-1(s_))
                if t[0][3] is True:
                    action_values[a] = t[0][0] * (t[0][2] + gamma * v_old[t[0][1]])
                else:
                    action_values[a] = t[0][0] * (t[0][2]+gamma*v_old[t[0][1]]) + \
                                        t[1][0] * (t[1][2]+gamma*v_old[t[1][1]]) + \
                                        t[2][0] * (t[2][2]+gamma*v_old[t[2][1]])
            v[s] = max(action_values)
            pi[s] = np.argmax(action_values)
        delta = 0
        for value in range(NUM_STATES):
            delta += abs(v[value] - v_old[value])
            # check sum of all deltas
        if delta < epsilon:
            return pi
        v_old = v.copy()
...
# policy evaluation
    ...
    for k in range(1, max_iterations):
        ...
        while True:
            delta = 0
            for s in range(NUM_STATES):
                t = TRANSITION_MODEL[s][pi[s]]
                small_v = v[s]
                # Vk(s) = (P(s_ | s, pi(s)) * R(s, a, s_) + gamma* V(s_))
                if t[0][3] is True:
                    v[s] = t[0][0] * (t[0][2] + gamma * v[t[0][1]])
                else:
                    v[s] = t[0][0] * (t[0][2] + gamma * v[t[0][1]]) + \
                            t[1][0] * (t[1][2] + gamma * v[t[1][1]]) + \
                            t[2][0] * (t[2][2] + gamma * v[t[2][1]])
                delta = max(delta, abs(small_v - v[s]))
            if delta < epsilon:
                break
# policy improvement
        policy_stable = True
        for s in range(NUM_STATES):
            ...
            for a in range(NUM_ACTIONS):
                t = TRANSITION_MODEL[s][a]
                # Vk(s) = max(P(s_ | s, a) * R(s, a, s_) + gamma* V(s_))
                if t[0][3] is True:
                    action_values[a] = t[0][0] * (t[0][2] + gamma * v_old[t[0][1]])
                else:
                    action_values[a] = t[0][0] * (t[0][2] + gamma * v_old[t[0][1]]) + \
...
def q_learning(env, gamma, max_iterations, logger):
    ...
    NUM_STATES = env.observation_space.n
    NUM_ACTIONS = env.action_space.n
    DECREASE_EVERY = max_iterations / 10
    
    v = [0] * NUM_STATES
    pi = [0] * NUM_STATES
    old_q = np.zeros([NUM_STATES, NUM_ACTIONS])
    q = np.zeros([NUM_STATES, NUM_ACTIONS])           # set all to 0
    # Visualize the initial value and policy
    logger.log(0, v, pi)
