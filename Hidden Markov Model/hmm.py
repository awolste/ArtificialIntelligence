...
class ExactInference(Inference):
    ...
    def observe(self,
        ...
        for i in range(0, self.n_rows):
            for j in range(0, self.n_cols):
                ...
                # find actual coordinate for the cell
                true_coord = self.get_coordinate(i, j)

                sensor_model = 1
                for landmark_num in range(0, len(landmarks)):
                    # calculate euclidean distance from each landmark to the given cell for each landmark
                    true_distance = math.sqrt(math.pow(true_coord[0] - landmarks[landmark_num][0], 2) +
                                              math.pow(true_coord[1] - landmarks[landmark_num][1], 2))
                    # create weights using pdf function with euclid dist, noise, and the obs dist for each landmark
                    sensor_model *= normal_pdf(true_distance, App.SENSOR_NOISE, observed_distances[landmark_num])

                self.belief[i][j] = sensor_model * self.belief[i][j]
        self.belief = self.normalize(self.belief)
    ...
    def timeUpdate(self):
        ...
        for i in range(0, self.n_rows):
            for j in range(0, self.n_cols):
                sum = 0
                # find each possible state for the tiger to go to
                for possible_state in self.transition_model(i, j):
                    # new = sum (trans model prob * current belief)
                    sum += possible_state[0] * \
                           self.belief[possible_state[1][0]][possible_state[1][1]]
                    # be sure not to update belief while still using
                    temp_belief[i][j] = sum
...
class ParticleFilter(Inference):
    ...
    def observe(self,
        ...
        weight_list = []
        # loop only the particles not each cell
        for i in self.particles:
            # find actual coordinate for the cell
            true_coord = self.get_coordinate(i[0], i[1])
            ...
            for landmark_num in range(0, len(landmarks)):
                ...
                true_distance = math.sqrt(math.pow(true_coord[0] - landmarks[landmark_num][0], 2) +
                                          math.pow(true_coord[1] - landmarks[landmark_num][1], 2))
                # create weights using pdf function with euclid dist, noise, and the obs dist for each landmark
                w_i *= normal_pdf(true_distance, App.SENSOR_NOISE, observed_distances[landmark_num])

            weight_list.append(w_i)

        # resample using weights and current particles and update
        self.particles = random.choices(self.particles, weights=weight_list, k=200)
        self.updateBelief()
    ...
    def timeUpdate(self):
        ...
        for i in self.particles:
            sum = 0
            rand = random.random()
            # for each possible state, if the accumulated probability > rand, choose that cell
            for possible_state in self.transition_model(i[0], i[1]):
                sum += possible_state[0]
                if rand < sum:
                    temp_particles.append(possible_state[1])
                    break
        # update
